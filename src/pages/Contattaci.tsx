...
const Contattaci = () => {
  ...
  const validate = () => {
    if (!name.trim() || !email.trim() || !message.trim()) {
      toast({ title: "Campi mancanti", description: "Compila nome, email e messaggio.", variant: "destructive" });
      return false;
    }
    ...
    if (message.trim().length < 10) {
      toast({ title: "Messaggio troppo breve", description: "Aggiungi qualche dettaglio in più.", variant: "destructive" });
      return false;
    }
    return true;
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      toast({ title: "Invio non riuscito", description: err?.message || "Riprova più tardi.", variant: "destructive" });
    } finally {
      setLoading(false);
    }
  };
  ...
  const content = (
    <div className="min-h-screen bg-gradient-to-br from-sky-50 via-indigo-50 to-purple-50">
      ...
      <div className="relative overflow-hidden">
        ...
        <PageTopBar className="relative z-10" fallbackPath="/" label="Torna alla Home" />
        <div className="max-w-5xl mx-auto px-4 md:px-6 pt-10 md:pt-14 pb-4">
          <PageHeader
            title="Contattaci"
            subtitle="Hai domande, suggerimenti o vuoi collaborare con noi? Scrivici!"
          />
        </div>
      </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-5">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                ...
                <Input id="name" value={name} onChange={(e) => setName(e.target.value)} placeholder="Il tuo nome" />
              </div>
              <div>
                <Label htmlFor="email">Email</Label>
                <Input id="email" type="email" value={email} onChange={(e) => setEmail(e.target.value)} placeholder="nome@esempio.com" />
              </div>
            </div>
            <div>
              <Label htmlFor="message">Messaggio</Label>
              <Textarea id="message" value={message} onChange={(e) => setMessage(e.target.value)} rows={6} placeholder="Come possiamo aiutarti?" />
            </div>
            {/* Honeypot */}
            ...
            <div className="flex items-center justify-between pt-2">
              <div className="flex items-center text-sm text-muted-foreground gap-2">
                ...
                <span>Rispondiamo in 24-48 ore lavorative</span>
              </div>
              <Button type="submit" disabled={loading} className="bg-gradient-to-r from-blue-600 to-purple-600 hover:from-purple-600 hover:to-blue-600">
                <Send className="h-4 w-4 mr-2" /> {loading ? 'Invio…' : 'Invia'}
              </Button>
            </div>
          </form>
        </div>
