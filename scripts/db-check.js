...
async function main() {
  ...
  // Conteggi minimi, se le tabelle non esistono non fallire
  async function countSafe(tbl) {
    try { const r = await client.query(`select count(*)::int as c from ${tbl}`); return r.rows[0].c; } catch { return null; }
  }
  const counts = {};
