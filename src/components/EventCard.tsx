...
const defaultFormatDate = (dateStr: string) => {
  try {
    ...
    return isValid(d) ? format(d, 'd MMM yyyy', { locale: itLocale }) : dateStr;
  } catch {
    return dateStr;
  }
};
...
const EventCard = ({ event, variant = "full", formatDate, showCategoryBadge = true, isPast = false }: EventCardProps) => {
  ...
  <Card className="overflow-hidden hover:shadow-lg transition-shadow duration-200 bg-white/90 backdrop-blur-sm border-0 shadow-md h-auto flex flex-col">
      {showImage && (
        <Link to={eventPath} className="relative overflow-hidden flex-shrink-0 block">
          <img
            src={imageUrl}
            alt={event.title}
            className="w-full aspect-[350/250] object-cover object-center md:object-[50%_35%] transition-transform duration-200 hover:scale-105"
      ...
      {!showImage && (
        <Link to={eventPath} className="relative aspect-[350/250] bg-gradient-to-br from-blue-100 to-purple-100 flex items-center justify-center flex-shrink-0 block">
          ...
          {event.fixed_appointment === true ? (
            ...
            event.date && (
              <div className="flex items-center gap-2 text-xs text-gray-600">
                <Calendar className="h-4 w-4 text-blue-500 flex-shrink-0" />
                <span>
                  {(formatDate ?? defaultFormatDate)(event.date)}
                  {event.end_date && event.end_date !== event.date && (
                    <>
                      {" - "}
                      {(formatDate ?? defaultFormatDate)(event.end_date)}
                    </>
                  )}
                </span>
              </div>
